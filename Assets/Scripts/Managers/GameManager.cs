...
public class GameManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Instance = this;
        // No DontDestroyOnLoad — scene-bound. GameManager is recreated each scene load.
        // GameProgressData persists via PlayerPrefs (see GameProgressData.cs).
    ...
    private IEnumerator ShowDayTransitionCoroutine(float durationSeconds)
    {
        ...
        if (dayTransitionContinueButton != null)
        {
            while (!dayTransitionContinueRequested && dayTransitionContinueButton != null)
            {
                yield return null;
            }
        }
